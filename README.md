...
- To run the program successfully, add the following environment variables (either through your Jetbrains IDE or through
  a terminal):
    ...
    - ``POSTGRES_TABLES`` and set it to **THE ABSOLUTE PATH** to the SQL
      ...
      at ``D:/Projects/ter_archivage/ter_backend/bin/database/database.sql``
        - I resided to this solution as ***unfortunately*** Dart does not support relative path resolving.
        - **If you find a fix to this issue, please let us know!**
    - ``FS_STORAGE_BASE_PATH`` and set it to the full local path to the directory you wish to store the files in. *
      *Default is: ``/mnt/ter_archivage_fs``**
...
- ``/user/create`` to create a User, this webservice takes:
    ...
    - ``password`` an encrypted string using **AES CTR** and the common key. (for now, this field is useless) (
      authentication isn't impelented yet.)
...
- ``/document/create`` to create a Document, this webservice takes:
    - ***This request must be sent as a ``Multipart Form-Data``***
...
>
> If you really need to generate a new key pair, you could use websites like: [this one](https://travistidwell.com/jsencrypt/demo/) or apps like: [PuTTYgen](https://www.puttygen.com/)
...
- ``/auth/attempt`` attempts to authenticate a user given some credentials, it takes:
    ...
    - ``encrypted_password`` the password to use, this password must be encrypted using the **RSA public key** obtained from ``/auth/key``
...
> [!CAUTION]
> A token generated by the back-end has **no set lifetime** *(meaning that it is valid forever)* unless either the user's password or username change.\
